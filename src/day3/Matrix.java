...
public class Matrix {
    ...
    public Matrix(List<char[]> matrixList, List<Coordinate> coordinates) {
    ...
    private List<Integer> findSurroundValue(Coordinate coordinate) {
       ...
       return coordinate.surround(MAX_LINE, MAX_COLUMN).stream().map(this::findNumber).toList();

    }

    public void findSurroundValues() {
        System.out.println(coordinates.stream()
                .flatMap(coordinate->this.findSurroundValue(coordinate).stream())
